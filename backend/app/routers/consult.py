...
def generate_plan(session_id: int,
                  db: Session = Depends(get_session),
                  uid: int = Depends(get_current_user_id)):
    # 1️⃣ 验证权限
    session_obj = db.get(ConsultSession, session_id)
    ...
    history_rows = db.exec(
        select(ChatMessage).where(
            ChatMessage.session_id == session_id).order_by(
    ...
    for item in new_advices:
        if not isinstance(item, dict): 
            continue
        title = item.get("title") or "建议"
        tags_list = item.get("tags") or []
        if tags_list:
            prefix = f"【{tags_list[0]}】"
            if not title.startswith(prefix):
                title = f"{prefix}{title}"
        reason = item.get("reason") or "问诊总结"
        detail_data = item.get("detail_json", {})
        advice = AdviceItem(
            user_id=uid,
            member_id=member.id,
            title=title,
    ...
    for item in new_tasks:
        ...
        # 👇👇👇 核心修复：从 AI 的结果里拿 repeating 👇👇👇
        is_repeating_task = item.get("repeating", False) 
